...
if ( ! function_exists( 'siqrp_deactivate_feedback' ) ) {

	/**
	 * Handles adding required code for the feedback form
	 */
	function siqrp_deactivate_feedback() {
		// Plugins.
		/**
		 ...
		 */
		$plugins = apply_filters( 'siqrp_deactivate_feedback_form_plugins', array() );
		...
		wp_localize_script(
			'shareaholic-deactivate-feedback-form',
			'siqrp_deactivate_feedback_form_plugins',
			$plugins
		);
	}
}

add_action( 'admin_enqueue_scripts', 'siqrp_deactivate_feedback' );

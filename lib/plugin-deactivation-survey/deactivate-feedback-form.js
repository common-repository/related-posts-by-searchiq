...
(function ($) {
	...
	shareaholic.DeactivateFeedbackForm.prototype.onSubmit = function (event) {
		...
		if ($( element ).find( "input[name='reason']:checked" ).length) {
			$.ajax(
				{
					type:        "POST",
					url:        "https://" + this.plugin.api_server + "/api/plugin_surveys", // More info about what and where the data is sent is under Development Tab on the plugin page on Wordpress.org.
	...
	$( document ).ready(
		function () {
			var plugins = siqrp_deactivate_feedback_form_plugins.length;
			for (var i = 0; i < plugins; i++) {
				var plugin = siqrp_deactivate_feedback_form_plugins[i];
